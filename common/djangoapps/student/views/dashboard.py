...
def get_course_enrollments(user, org_whitelist, org_blacklist, request=None):
    """
    ...
    """
    for enrollment in CourseEnrollment.enrollments_for_user_with_overviews_preload(user, request):
...
def student_dashboard(request):
    ...
    course_enrollments = list(get_course_enrollments(user, site_org_whitelist, site_org_blacklist, request))
    ...
    # Check whether we are showing all the courses or not
    load_all_courses = False
    if 'load_all_course' in request.GET:
        load_all_courses = True
    ...
    context = {
        ...
        'courses_count': CourseEnrollment.get_dashboard_courses_count(),
